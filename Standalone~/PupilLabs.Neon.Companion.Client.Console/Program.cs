namespace PupilLabs.Neon.Companion.Client.ConsoleApp
{
    ...
    internal static class Program
    {
        private sealed class Options
        {
            ...
            [Option("run-lsl", Required = false, Default = false, HelpText = "run lsl")]
            public bool RunLsl { get; set; }
        }
        ...
        private static async Task RunAsync(Options options)
        {
            ...
            if (options.RunGetStatus || options.RunTimeEcho)
            {
            ...
            if (options.RunLsl)
            {
                LSLHelper.GetLocalClock();
                LSLHelper.ListAllStreams();
